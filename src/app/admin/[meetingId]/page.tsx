...
export default function MeetingDetails() {
  ...
  useEffect(() => {
    ...
    // Real-time Listen for Attendees
    const q = query(
  ...
  const exportToCSV = () => {
    ...
    const headers = ['Full Name', 'Email', 'Mobile', 'Company', 'Status', 'Consent', 'Timestamp'];
    ...
    link.setAttribute('download', `attendance_${meeting?.name.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: '1.5rem', color: 'var(--text-muted)' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Calendar size={18} /> {meeting?.date}
            </span>
            <span style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
              <Users size={18} /> {attendees.length} Attendees
            </span>
          </div>
        </div>
            ...
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead>
                <tr style={{ textAlign: 'left', borderBottom: '1px solid var(--glass-border)' }}>
                  <th style={{ padding: '1rem', color: 'var(--text-muted)', fontSize: '0.875rem' }}>Name</th>
                  <th style={{ padding: '1rem', color: 'var(--text-muted)', fontSize: '0.875rem' }}>Company</th>
                  <th style={{ padding: '1rem', color: 'var(--text-muted)', fontSize: '0.875rem' }}>Status</th>
                  <th style={{ padding: '1rem', color: 'var(--text-muted)', fontSize: '1rem' }}>Share</th>
                </tr>
              </thead>
              <tbody>
                {attendees.map((attendee) => (
                  <tr key={attendee.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                    <td style={{ padding: '1rem' }}>
                      <div style={{ fontWeight: '600' }}>{attendee.fullName}</div>
                      <div style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>{attendee.email}</div>
                    </td>
                    <td style={{ padding: '1rem', color: 'var(--text-muted)' }}>{attendee.company || '-'}</td>
                    <td style={{ padding: '1rem' }}>
                    ...
                    <td style={{ padding: '1rem' }}>
                       {attendee.consent ? <Share2 size={14} color="var(--success)" /> : <span style={{ opacity: 0.3 }}>-</span>}
                    </td>
                  </tr>
                ))}
                {attendees.length === 0 && (
